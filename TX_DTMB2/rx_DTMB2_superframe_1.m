...
for SNR_IN = SNR %定义输入信噪比
    ...
    mmse_weight = 0.99;
    if debug_lmmse
        noise_off = Send_data_srrc_tx1_ch - Send_data_srrc_tx1_ch_spn;
        lmmse_SNR = norm(noise_off)/norm(data_transfer);
    end
    ...
    %单PN信道估计
     for i=1:sim_num-1
          ...
          if i > 1
              ...
              if debug_lmmse
                  denominator =  conj(last_frame_h_freq).*last_frame_h_freq+lmmse_SNR;
                  last_frame_ofdm_eq =  (conj(last_frame_ofdm_freq).*last_frame_h_freq)./denominator;
              else
